...
或者我们套用另一种模式，即123原解法一的模式，将n天划分为k个区间，每个区间最多交易一次。
f[i][j] 代表第i天为止交易k次获得的最大收益，那么将问题分解为前 x 天交易 k-1 次，第 x+1 天至第 i 天交易一次两个子问题，于是动态方程如下：
...
该解法的时间复杂度高达O(n
​2
​​ ⋅k)。似乎是由于无法进行memoization而导致大量的重复计算。